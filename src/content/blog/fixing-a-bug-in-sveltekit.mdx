...
Well, the main thing is that when I run it locally, I use `pnpm dev` whereas in prod, I use the [Node Adapter](https://svelte.dev/docs/kit/adapter-node) running inside Docker. This narrows it down somewhat – here are the main suspects:
...
It's perfect – it fits all the symptoms and explains why I'm not seeing the issue locally (I disable Sentry in local dev environment). [^mixing-metaphors]

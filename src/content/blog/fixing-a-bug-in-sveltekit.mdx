---
title: Fighting Proxy Wars in SvelteKit
description: "How I found and fixed a fun bug in SvelteKit"
date: 2025-12-03
tags:
  - svelte
socials: []
...
Hopefully, someone wil find this useful if they come across a similar issue – there didn't seem to be a lot around it when I was frantically Googling it during the early phases of "why is this happening to me???", anyway. (That includes me in 3 months time when I've forgotten all about this.)
...
Well, the main thing is that when I run it locally, I use `pnpm dev` whereas in prod, I use the [Node Adapter](https://svelte.dev/docs/kit/adapter-node) running insidemore than once Docker. This narrows it down somewhat – here are the main suspects:
...
This is all a bit weird as the actual error is happening inside an internal node dependency, [undici](https://github.com/nodejs/undici). I haven't bumped by Node version itself so this clue confuses me greatly.
...
It's prefect – it fits all the symptoms and explains why I'm not seeing the issue locally (I disable Sentry in local dev environment). [^mixing-metaphors]

[^mixing-metaphors]: I am mixing the metaphors a little here – first it's a murder, now it's a diagnostic mystery – just stick with it.
...
<Aside type="tip">Don't be silly and accidentally put this code somewhere where it runs on every request, or you'll end up with expontential explosion of `console.trace` calls as each new proxy triggers the other proxies and adds another trace to the pile, like the logging equivalent of a nuclear bomb... Not that I did that or anything, that would be stupid haha...</Aside>
...
[^reflect-get]: If you're wondering what the difference is between `obj[key]` and `Reflect.get(obj, key, obj)`, it only makes a difference when `key` has a getter defined on `obj` which means that doing `obj[key]` actually invokes the getter method – `Reflect.get()` ensures that `this` is correctly bound to `obj` when the getter is invoked.
...
In the CI pipeline, it was Node v24 that was being pulled instead of v22 – the `#state` private field was introduced in v24 so that it why the issue was not showing before.
...
It is at this point that I found out that pnpm has it's own node version manager built-in, which you can change using `pnpm use --global 24`. If you want to be sure, you can do `pnpm exec node --version` to tell you.
...
```js frame="shell"
» brokenProxy.status("Drew", "drew@person.example")
...
» plainProxy.status("Drew", "drew@person.example")
← "I am Drew <drew@person.example> and I am hungry"
...
» fancyProxy.status("Drew", "drew@person.example")
← "I am Drew <drew@person.example> and I am hungry"
...
As expected, the original proxy is broken as `this` is not bound and so the privacy of the `Person.#hunger` field is violated. Both the plain and fancy proxies work when invoked, when they you look at their name and length (the latter being the number of arguments), they are different.
...
I created a [PR with this fix](https://github.com/sveltejs/kit/pull/15005) whereupon it was quickly merged and within 4 days it was bundled into the next SvelteKit release – this project is really actively maintained.

...
I use <Link external href="https://tableplus.com/">TablePlus</Link> for my database management - it's got a nice, clean GUI that feels both modern and intuitive and also not incredibly bloated like the other ones (\*\*cough\*_ DataGrip _\*cough\*\*).

## Top CLI tools

I listed a few CLI tools already, here's a list of my favourite CLI tools that I use on a daily basis:

- <Link external href="https://fishshell.com/">
    Fish Shell
  </Link> - you can get the same kind of setup from zsh, but you have to put a lot
  more effort in. With Fish, it all comes out of the box. Also I like the more modern
  shell scripting syntax.
- <Link external href="https://eza.rocks/">
    eza
  </Link> - we're well into the 21st century now, why do we still need to use the
...
- <Link external href="https://starship.rs">
    Starship
  </Link> - a prompt that is pretty, unobtrusive and only tells you the things it
...
- <Link external href="https://jqlang.github.io/jq/">
    jq
  </Link> - a JSON processor, useful for formatting JSON output from APIs and for
  manipulating JSON objects so that you can pass them around between scripts or iterate
  over them.
- <Link external href="https://github.com/sharkdp/fd">
    fd
  </Link> - like `find` but a billion times faster, and more intuitive to use.
- <Link external href="https://github.com/sharkdp/bat">
    bat
  </Link> - like `cat` but with syntax highlighting and line numbers.
- <Link external href="https://github.com/sharkdp/hexyl">
    hexyl
  </Link> - command-line hex viewer, with syntax highlighting.
- <Link external href="https://github.com/BurntSushi/ripgrep">
    ripgrep
  </Link> - like `grep` but much faster and doesn't feel like it's from the 90s.
- <Link external href="https://github.com/dandavison/delta">
    delta
  </Link> - a better differ for git, with more human readable output, syntax highlighting,
  etc.
- <Link external href="https://taskfile.dev/">
    task
  </Link> - stop writing makefiles for Go projects (in fact, stop writing makefiles
  ...
  way.
- <Link external href="https://github.com/dbrgn/tealdeer">
    tealdeer
  </Link> - 95% of the time when I don't know how to use a CLI tool, I just want
  to know the most common use cases. <Link external href="https://tldr.sh/">
    tldr
  </Link> is a collaborative community-driven effort to simplify man pages into something
  that I can quickly read and understand. Tealdeer is a CLI interface to the tldr
  repository.
- <Link external href="https://github.com/ClementTsang/bottom">
    bottom
  </Link> - like `top` but better. I've tried a few of these and this is the one
  I like the most.
- <Link external href="https://atuin.sh/">
    atuin
  </Link> - sync your shell history across multiple machines. I have disabled the
  up arrow and enabled the ctrl-r search, so really I'm just using it so I can search
  my shell history from any machine.
- <Link external href="https://rye.astral.sh/guide/installation/">
    rye
  </Link> - rye is like pyenv and Poetry bundled together. It's also really fast,
  which is nice. It uses uv for package management which makes it insanely fast to
  install Python packages (no really, it is). It's from the same guy who created <Link
    external
    href="https://astral.sh/ruff"
  >
    ruff
  </Link> and <Link external href="https://github.com/astral-sh/uv">
    uv
  </Link>
  , both excellent tools.
- <Link external href="https://dev.yorhel.nl/ncdu">
    ncdu
  </Link> - a disk usage analyzer with an ncurses interface.
